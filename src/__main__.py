...
while True:
    ...
    for result in results:
        ...
        if (
            result["reg"] not in latest_flights.keys()
            or not latest_flights[result["reg"]]
        ):
    ...
    for reg, latest_id in latest_flights.copy().items():
        ...
        if cur.rowcount == 0:
            ...
            for new_flight in results:
                ...
                print(
                    f"\t New {new_flight['id']}, {new_flight['origin']} -> {new_flight['destination']}, landed {hours} hours : {minutes} mins ago"
                )
                origin_airport = get_airport_by_icao(cur, new_flight["origin"])
                destination_airport = get_airport_by_icao(
                    cur, new_flight["destination"]
                )
                notify(
                    cur,
                    new_flight,
