...
while True:
    ...
    AND f.landing_time < (CURRENT_TIMESTAMP - INTERVAL '24 HOURS')
    and f.reg = %(reg)s
    ORDER BY f.id ASC
    """

    # Execute the query for each aircraft in the dictionary
    for reg, latest_id in latest_flights.items():
        print(f"Checking {reg} for flights to post:", end=" ")
        ...
        if cur.rowcount == 0:
            ...
            for new_flight in results:
                ...
                notify(cur, new_flight, origin_airport, destination_airport, twitter_details)
